...
data "aws_iam_policy_document" "docs_deploy" {
  ...
  # No S3 statement is needed to fetch the staged HTML docs: the staging bucket
  # grants public s3:GetObject via its bucket policy, and deploy_docs.yml reads
  # the htmldocs by its exact key (no ListBucket / --recursive).
}
